...
function hydrateWifiState(payload) {
  ...
  state.wifi.staSsid = payload.sta_ssid ?? state.wifi.staSsid ?? "";
  state.wifi.staIp = payload.sta_ip ?? state.wifi.staIp ?? "";
  state.wifi.staConnected = payload.sta_connected ?? state.wifi.staConnected ?? false;
  fillWifiForm();
}
...
async function fetchWifiInfo() {
  try {
    ...
    const apLine = `[WIFI] 本地热点: ${state.wifi.ssid}${state.wifi.open ? " (开放)" : ""} @ ${state.wifi.ip || "未知"}`;
    const staLine = state.wifi.staSsid
      ? `园区网: ${state.wifi.staSsid} ${state.wifi.staConnected ? "已连接" : "未连接"}${state.wifi.staIp ? " · " + state.wifi.staIp : ""}`
      : "";
    appendLog(staLine ? `${apLine}\n${staLine}` : apLine);
  } catch (e) {
